...
//TODO: just get rid of 2 unused functions
...
public class GameBoard {
    ...
    public ArrayList<Move> getPossibleMoves(int r, int c){
        ...
        for(int[] d : directions){
            ...
            //      - not out of bounds
            //      - enemy in between
            if( !outOfBounds(row, col) && !outOfBounds(jumpRow, jumpCol) && board[jumpRow][jumpCol] == Piece.EMPTY && isEnemy(p, board[row][col])){
    ...
    public ArrayList<Move> getPlayerMoves(Piece player){
        ...
        for (int r = 0; r < 8; r++) {
            for (int c = 0; c < 8; c++) {
                ...
                if(!isPlayer(player, p)) continue; //if it's the other player's piece, move on
